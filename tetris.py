import pygame
import sys
import random
import os
import json
from datetime import datetime
import threading
import asyncio
import websockets

# --- DIT TOEVOEGEN ---
# dit is nodig als je een executable app maakt met PyInstaller, 
# zodat het programma altijd in de juiste map werkt, 
# ongeacht of het een .app of een script is.
if getattr(sys, 'frozen', False):
    # Als het een .app is, ga naar de tijdelijke PyInstaller map
    os.chdir(sys._MEIPASS)
else:
    # Als het een script is, blijf in de huidige map
    os.chdir(os.path.dirname(os.path.abspath(__file__)))
# ----------------------

# --- 1. INITIALISATIE ---
pygame.init()
pygame.font.init() # Expliciet font initialiseren voor de zekerheid 

# --- CONFIGURATIE & CONSTANTEN ---
WIDTH, HEIGHT = 300, 600
COLS, ROWS = 10, 20
CELL_SIZE = WIDTH // COLS
SIDEBAR_WIDTH = 150
TOTAL_WIDTH = WIDTH + SIDEBAR_WIDTH

BLACK = (0, 0, 0)
DARK_GREY = (40, 40, 40)
WHITE = (255, 255, 255)
RED = (255, 0, 0)
GREEN = (0, 255, 0)
YELLOW = (255, 255, 0)

# Lettertypes laden
FONT_UI = pygame.font.SysFont('Arial', 24, bold=True)

# Tetromino definities
SHAPES = {
    'I': [[1, 1, 1, 1]],
    'O': [[1, 1], [1, 1]],
    'T': [[0, 1, 0], [1, 1, 1]],
    'S': [[0, 1, 1], [1, 1, 0]],
    'Z': [[1, 1, 0], [0, 1, 1]],
    'J': [[1, 0, 0], [1, 1, 1]],
    'L': [[0, 0, 1], [1, 1, 1]]
}

COLORS = {
    'I': (0, 200, 200), 'O': (200, 200, 0), 'T': (150, 0, 150),
    'S': (0, 200, 0), 'Z': (200, 0, 0), 'J': (0, 0, 200), 'L': (200, 100, 0)
}

# Een globale lijst om commando's van de ESP32 in op te slaan
remote_commands = []

# --- FUNCTIES ---

def get_new_piece():
    shape_type = random.choice(list(SHAPES.keys()))
    return {
        'matrix': SHAPES[shape_type],
        'color': COLORS[shape_type],
        'x': COLS // 2 - len(SHAPES[shape_type][0]) // 2,
        'y': 0
    }

def is_valid_move(piece_matrix, grid, x_pos, y_pos):
    for y, row in enumerate(piece_matrix):
        for x, cell in enumerate(row):
            if cell:
                new_x = x_pos + x
                new_y = y_pos + y
                if new_x < 0 or new_x >= COLS or new_y >= ROWS:
                    return False
                if new_y >= 0 and grid[new_y][new_x]:
                    return False
    return True

def lock_piece(piece, grid):
    for y, row in enumerate(piece['matrix']):
        for x, cell in enumerate(row):
            if cell:
                grid[piece['y'] + y][piece['x'] + x] = piece['color']

def clear_rows(grid):
    new_grid = [row for row in grid if any(cell is None for cell in row)]
    rows_cleared = ROWS - len(new_grid)
    for _ in range(rows_cleared):
        new_grid.insert(0, [None for _ in range(COLS)])
    return new_grid, rows_cleared

def rotate(matrix):
    return [list(row) for row in zip(*matrix[::-1])]

def draw_game(screen, grid, current_piece, next_piece, held_piece):
    screen.fill(BLACK)

    # 1. Grid lijnen - het speelveld tekenen
    for y in range(ROWS):
        for x in range(COLS):
            rect = pygame.Rect(x * CELL_SIZE, y * CELL_SIZE, CELL_SIZE, CELL_SIZE)
            pygame.draw.rect(screen, DARK_GREY, rect, 1)
    
    for y, row in enumerate(grid):
        for x, color in enumerate(row):
            if color:
                pygame.draw.rect(screen, color, (x * CELL_SIZE, y * CELL_SIZE, CELL_SIZE - 1, CELL_SIZE - 1))
    
    # 2. DE VALLENDE TETROMINO
    # Deze moet getekend worden op de x en y positie van het speelveld
    for y, row in enumerate(current_piece['matrix']):
        for x, cell in enumerate(row):
            if cell:
                px = (current_piece['x'] + x) * CELL_SIZE
                py = (current_piece['y'] + y) * CELL_SIZE
                pygame.draw.rect(screen, current_piece['color'], (px, py, CELL_SIZE - 1, CELL_SIZE - 1))

    # 3. Sidebar achtergrond
    pygame.draw.rect(screen, (20, 20, 20), (WIDTH, 0, SIDEBAR_WIDTH, HEIGHT))

    # --- NEXT Section ---
    text_next = FONT_UI.render('NEXT', True, WHITE)
    screen.blit(text_next, (WIDTH + 20, 20))
    # Actieve piece
    for y, row in enumerate(next_piece['matrix']):
        for x, cell in enumerate(row):
            if cell:
                px = WIDTH + 30 + (x * CELL_SIZE)
                py = 70 + (y * CELL_SIZE)
                pygame.draw.rect(screen, next_piece['color'], (px, py, CELL_SIZE - 1, CELL_SIZE - 1))

    # --- HOLD Section ---
    text_hold = FONT_UI.render('HOLD', True, WHITE)
    screen.blit(text_hold, (WIDTH + 20, 200)) # Iets lager dan NEXT
    
    if held_piece:
        for y, row in enumerate(held_piece['matrix']):
            for x, cell in enumerate(row):
                if cell:
                    px = WIDTH + 30 + (x * CELL_SIZE)
                    py = 240 + (y * CELL_SIZE)
                    pygame.draw.rect(screen, held_piece['color'], (px, py, CELL_SIZE - 1, CELL_SIZE - 1))

def draw_score(screen, score, level):
    # Score weergeven
    label = FONT_UI.render('SCORE', True, WHITE)
    screen.blit(label, (WIDTH + 20, 420))
    val = FONT_UI.render(str(score), True, GREEN)
    screen.blit(val, (WIDTH + 20, 450))

    # Level weergeven
    label = FONT_UI.render('LEVEL', True, WHITE)
    screen.blit(label, (WIDTH + 20, 520))
    val = FONT_UI.render(str(level), True, GREEN)
    screen.blit(val, (WIDTH + 20, 550))

def draw_game_over(screen, final_score, highscores):
    overlay = pygame.Surface((TOTAL_WIDTH, HEIGHT), pygame.SRCALPHA)
    overlay.fill((0, 0, 0, 230)) 
    screen.blit(overlay, (0, 0))
    
    msg = pygame.font.SysFont('Arial', 40, bold=True).render('HALL OF FAME', True, YELLOW)
    screen.blit(msg, (TOTAL_WIDTH // 2 - msg.get_width() // 2, 30))
    
    # Header voor kolommen
    header_txt = pygame.font.SysFont('Arial', 18, bold=True).render("NAME      SCORE      DATE", True, WHITE)
    screen.blit(header_txt, (30, 90))
    
    for i, entry in enumerate(highscores):
        # Markeer de huidige score in het groen
        color = GREEN if entry["score"] == final_score else WHITE
        
        # Format: Naam links, Score midden, Datum rechts
        name_txt = f"{i+1}. {entry['name']}"
        score_txt = str(entry['score'])
        date_txt = entry['date']
        
        # Teken de tekst (je kunt hier met x-posities spelen voor mooie kolommen)
        screen.blit(FONT_UI.render(name_txt, True, color), (30, 120 + i * 30))
        screen.blit(FONT_UI.render(score_txt, True, color), (160, 120 + i * 30))
        screen.blit(pygame.font.SysFont('Arial', 16).render(date_txt, True, color), (250, 125 + i * 30))

    # Instructies
    retry_txt = FONT_UI.render("Press 'R' to Restart", True, WHITE)
    quit_txt = FONT_UI.render("Press 'Q' to Quit", True, WHITE)
    screen.blit(retry_txt, retry_txt.get_rect(center=(TOTAL_WIDTH // 2, HEIGHT - 80)))
    screen.blit(quit_txt, quit_txt.get_rect(center=(TOTAL_WIDTH // 2, HEIGHT - 40)))

def init_audio():
    import sys
    import os
    
    try:
        if not pygame.mixer.get_init():
            pygame.mixer.init()

        # DE FIX VOOR MAC .APP BUNDELS:
        if getattr(sys, 'frozen', False):
            # In een .app bundel staan de bestanden in Contents/Resources
            # Maar PyInstaller zet ze in de root van de tijdelijke map
            base_path = sys._MEIPASS
        else:
            base_path = os.path.dirname(os.path.abspath(__file__))

        # Definieer de paden direct
        music_path = os.path.join(base_path, "Tetris.mp3")
        sound_path = os.path.join(base_path, "clear.wav")

        # Check 1: Muziek
        if os.path.exists(music_path):
            pygame.mixer.music.load(music_path)
            pygame.mixer.music.play(-1)
            pygame.mixer.music.set_volume(0.3)
        else:
            # Als hij daar niet staat, probeer de Resources map (backup)
            backup_path = os.path.join(base_path, "Contents", "Resources", "Tetris.mp3")
            if os.path.exists(backup_path):
                pygame.mixer.music.load(backup_path)
                pygame.mixer.music.play(-1)

        # Check 2: Sound effect
        clear_sfx = None
        if os.path.exists(sound_path):
            clear_sfx = pygame.mixer.Sound(sound_path)
            
        return clear_sfx

    except Exception:
        return None

# --- HIGHSCORE MANAGEMENT ---
# Deze functies beheren het laden, opslaan en bijwerken van highscores in een JSON-bestand.

# Bij het behalen van een nieuwe highscore, wordt de speler gevraagd om zijn naam in te voeren. 
# De highscores worden opgeslagen als een lijst van dictionaries met 'name', 'score' en 'date' velden.
# we moeten dit zo doen omdat pygame geen standaard tekstveld krijgt, 
# dus we simuleren dit met een eenvoudige loop die invoer accepteert.
def get_player_name(screen):
    name = ""
    asking = True
    while asking:
        screen.fill(BLACK)
        # Teken een simpel tekstveld-scherm
        prompt = FONT_UI.render("Nieuwe Highscore! Typ je naam:", True, WHITE)
        name_surface = FONT_UI.render(name + "_", True, YELLOW)
        
        screen.blit(prompt, (TOTAL_WIDTH // 2 - prompt.get_width() // 2, HEIGHT // 2 - 50))
        screen.blit(name_surface, (TOTAL_WIDTH // 2 - name_surface.get_width() // 2, HEIGHT // 2))
        pygame.display.flip()

        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN and name != "":
                    asking = False
                elif event.key == pygame.K_BACKSPACE:
                    name = name[:-1]
                else:
                    # Voeg alleen letters/cijfers toe en hou het kort
                    if len(name) < 10 and event.unicode.isalnum():
                        name += event.unicode
    return name

def load_highscores():
    if os.path.exists('highscores.json'):
        with open('highscores.json', 'r') as f:
            return json.load(f)
    return []

def save_highscores(scores):
    with open('highscores.json', 'w') as f:
        json.dump(scores, f)

def update_highscores(new_score, player_name=None):
    scores = load_highscores()
    current_time = datetime.now().strftime("%Y-%m-%d")

    new_entry = {
        "name": player_name,
        "score": new_score,
        "date": current_time
    }

    scores.append(new_entry)
    scores.sort(key=lambda x: x['score'], reverse=True)  # Sorteer op score, hoogste eerst
    scores = scores[:10]  # Behou alleen de top 10
    save_highscores(scores)
    return scores

# --- WEBSOCKET SERVER ---
# Deze server luistert naar inkomende berichten van de ESP32 en 
# voegt ze toe aan de remote_commands lijst.
async def server_handler(websocket, path=None):
    """Ontvangt berichten van de ESP32 en zet ze in de commandolijst."""
    try:
        async for message in websocket:
            # We verwachten berichten als "LEFT", "RIGHT", "UP", "DOWN"
            remote_commands.append(message.strip().upper())
    except Exception as e:
        print(f"Websocket error: {e}")

def start_websocket_server():
    """Start de server in een aparte thread met een eigen event loop."""
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    
    # Gebruik een context manager om de server correct te starten in de nieuwe loop
    async def run_server():
        async with websockets.serve(server_handler, "0.0.0.0", 8765):
            await asyncio.Future()  # Dit houdt de server eeuwig draaiend

    loop.run_until_complete(run_server())

# Start de server thread direct bij het laden van het script
ws_thread = threading.Thread(target=start_websocket_server, daemon=True)
if sys.platform == 'win32':
    asyncio.set_event_loop_policy(asyncio.WindowsSelectorEventLoopPolicy())

ws_thread.start()

def handle_remote_commands(current_piece, grid, game_over):
    """Verwerkt alle commando's die in de wachtrij van de ESP32 staan."""
    while len(remote_commands) > 0:
        cmd = remote_commands.pop(0)
        if not game_over:
            if cmd == "LEFT":
                if is_valid_move(current_piece['matrix'], grid, current_piece['x'] - 1, current_piece['y']):
                    current_piece['x'] -= 1
            elif cmd == "RIGHT":
                if is_valid_move(current_piece['matrix'], grid, current_piece['x'] + 1, current_piece['y']):
                    current_piece['x'] += 1
            elif cmd == "ROTATE":
                rotated = rotate(current_piece['matrix'])
                if is_valid_move(rotated, grid, current_piece['x'], current_piece['y']):
                    current_piece['matrix'] = rotated
            elif cmd == "DOWN":
                if is_valid_move(current_piece['matrix'], grid, current_piece['x'], current_piece['y'] + 1):
                    current_piece['y'] += 1
            elif cmd == "HOLD":
                return "HOLD"        
        elif cmd == "RESTART":
            return "RESTART"
    return None

def handle_player_input(current_piece, grid, held_piece, next_piece, can_hold):
    """Verwerkt toetsenbord-input en geeft de gewenste valsnelheid terug."""
    speed = None
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            return "QUIT", None, current_piece, held_piece, next_piece, can_hold
        
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_DOWN: speed = "FAST"
            if event.key == pygame.K_LEFT:
                if is_valid_move(current_piece['matrix'], grid, current_piece['x'] - 1, current_piece['y']):
                    current_piece['x'] -= 1
            if event.key == pygame.K_RIGHT:
                if is_valid_move(current_piece['matrix'], grid, current_piece['x'] + 1, current_piece['y']):
                    current_piece['x'] += 1
            if event.key == pygame.K_UP:
                rotated = rotate(current_piece['matrix'])
                if is_valid_move(rotated, grid, current_piece['x'], current_piece['y']):
                    current_piece['matrix'] = rotated
            # HOLD functie
            if event.key == pygame.K_c:
                current_piece, held_piece, next_piece, can_hold = handle_hold(
                    current_piece, held_piece, next_piece, grid, can_hold
                )   
        
        if event.type == pygame.KEYUP and event.key == pygame.K_DOWN:
            speed = "NORMAL"
            
    return "CONTINUE", speed, current_piece, held_piece, next_piece, can_hold

def update_game_physics(current_piece, next_piece, grid, score, level, lines_total, clear_sound, can_hold):
    """Berekent wat er gebeurt als een blokje valt of vast komt te zitten."""
    game_over = False
    
    if is_valid_move(current_piece['matrix'], grid, current_piece['x'], current_piece['y'] + 1):
        current_piece['y'] += 1
    else:
        lock_piece(current_piece, grid)
        can_hold = True  # Reset hold mogelijkheid bij elke val
        grid, removed = clear_rows(grid)
        
        if removed > 0:
            if clear_sound: clear_sound.play()
            score += {1: 100, 2: 300, 3: 500, 4: 800}.get(removed, 0)
            lines_total += removed
            level = (lines_total // 10) + 1
            
        current_piece = next_piece
        next_piece = get_new_piece()
        
        if not is_valid_move(current_piece['matrix'], grid, current_piece['x'], current_piece['y']):
            game_over = True
            
    return current_piece, next_piece, grid, score, level, lines_total, game_over, can_hold

def manage_game_over(screen, score, highscore_saved, highscores_list):
    """Regelt de naam-input en het opslaan van de score zodra het spel stopt."""
    if not highscore_saved:
        pygame.mixer.music.stop()
        player_name = get_player_name(screen)
        highscores_list = update_highscores(score, player_name)
        highscore_saved = True
    
    # Check voor herstart/afsluiten via toetsenbord
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            return False, True, highscores_list # running = False
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_r:
                return "RESTART", highscore_saved, highscores_list
            if event.key == pygame.K_q:
                return False, True, highscores_list # running = False
                
    return True, highscore_saved, highscores_list # running = True

def handle_hold(current_piece, held_piece, next_piece, grid, can_hold):
    if not can_hold:
        return current_piece, held_piece, next_piece, False

    if held_piece is None:
        # Eerste keer hold: bewaar huidige, pak de volgende uit de wachtrij
        held_piece = current_piece
        current_piece = next_piece
        next_piece = get_new_piece()
    else:
        # Wissel huidige met wat in hold stond
        held_piece, current_piece = current_piece, held_piece
    
    # Reset positie van het nieuwe blokje naar de top
    current_piece['x'] = COLS // 2 - len(current_piece['matrix'][0]) // 2
    current_piece['y'] = 0
    
    return current_piece, held_piece, next_piece, False # can_hold wordt False

# --- MAIN ENGINE ---

def main():
    screen = pygame.display.set_mode((TOTAL_WIDTH, HEIGHT))
    pygame.display.set_caption("Tetris - Frank's Versie")
    clock = pygame.time.Clock()

    clear_sound = init_audio() 

    # Variabelen initialiseren
    fall_time = 0
    normal_speed = 500
    fast_speed = 50
    current_fall_speed = normal_speed
    score = 0
    game_over = False
    level = 1
    lines_total = 0  # <--- Veranderd van lines_cleared_total naar lines_total

    grid = [[None for _ in range(COLS)] for _ in range(ROWS)]
    current_piece = get_new_piece()
    next_piece = get_new_piece()

    highscore_saved = False 
    highscores_list = load_highscores() 

    held_piece = None
    can_hold = True  # Wordt False zodra je wisselt, wordt weer True als een blokje landt
    
    running = True
    while running:
        dt = clock.tick(60)

        # 1. ESP32 Commando's verwerken
        remote_res = handle_remote_commands(current_piece, grid, game_over)
        if remote_res  == "RESTART":
            main()
            return
        elif remote_res == "HOLD":
            current_piece, held_piece, next_piece, can_hold = handle_hold(
                current_piece, held_piece, next_piece, grid, can_hold
            )

        if game_over:
            # 2. Game Over Flow (Naam invoeren & Highscores)
            status, highscore_saved, highscores_list = manage_game_over(screen, score, highscore_saved, highscores_list)
            if status == "RESTART": 
                main()
                return
            if status == False: 
                running = False
        else:
            # 3. Physics & Input
            fall_time += dt
            status, speed_change, current_piece, held_piece, next_piece, can_hold = handle_player_input(
                current_piece, grid, held_piece, next_piece, can_hold
            )
            
            if status == "QUIT": 
                running = False
            
            # Snelheid aanpassen op basis van input
            if speed_change == "FAST": 
                current_fall_speed = fast_speed
            elif speed_change == "NORMAL": 
                current_fall_speed = max(100, 500 - (level - 1) * 50)

            # snelheid aanpassen op basis van level
            level_speed = max(100, 500 - (level - 1) * 50)
            
            if speed_change == "FAST":
                current_fall_speed = fast_speed
            else:
                current_fall_speed = level_speed

            # Zwaartekracht update
            if fall_time > current_fall_speed:
                # Hier gebruiken we nu overal 'lines_total'
                current_piece, next_piece, grid, score, level, lines_total, game_over, can_hold = update_game_physics(
                    current_piece, next_piece, grid, score, level, lines_total, clear_sound, can_hold
                )
                fall_time = 0

        # --- RENDERING ---
        draw_game(screen, grid, current_piece, next_piece, held_piece)
        draw_score(screen, score, level)

        if game_over:
            draw_game_over(screen, score, highscores_list)

        pygame.display.flip()

    pygame.quit()
    sys.exit()

if __name__ == "__main__":
    main()